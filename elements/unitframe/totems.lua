    ...
    H.CastSpell                 = CastSpell
    H.CastSpellByName   = CastSpellByName
    H.UseIventoryItem       = UseIventoryItem
    ...
    local AddButtons = function()
        ...
        for i, v in pairs(
            ...
            v.timer:SetPoint('TOP', v, 'BOTTOM', 0, -2)
            v.timer:SetFont(STANDARD_TEXT_FONT, 10)
            v.timer:SetJustifyH'CENTER'
        end
    end
    
    local PositionTotems = function()
        local i = 1
        for _, v in pairs(
            {
                _G['modTotemEarth'],
    ...
    local AddUseAction = function(id, book)
        ...
        if isUsable and not notEnoughMana then
            _G['modTotemTooltip']:SetAction(id)
