    ...
    local var = MODUI_VAR['aura']['format']
    local H = {}
    ...
    function SecondsToTimeAbbrev(time, f)  -- AURA DURATION SUB
        ...
        elseif time < 3600 and time > 60 then
            h = floor(time/3600)
            m = (f ~= nil or var) and floor(mod(time, 3600)/60) or floor(mod(time, 3600)/60 + 1)
            s = mod(time, 60)
            text = (f ~= nil or var) and format(' |cffffffff%d|r:|cffffffff%02d|r', m, s) or format('|cffffffff%d|rm', m)
        elseif time < 60 then
            m = floor(time/60)
            s = mod(time, 60)
